...
class Board {
	private:
		...
		uint64_t frontier; // 0 = does not touch piece, 1 = touches piece
		...
		void setFrontier(uint8_t x, uint8_t y, bool value) {
            if (value) {
			    frontier |= (1ULL << (8*y + x));
            } else {
                frontier &= ~(1ULL << (8*y + x)); 
        ...
        bool getFrontier(uint8_t x, uint8_t y) {
            return  (frontier >> (8*y + x)) & 1;
        }
	...
	public:
		...
		Board() {
			...
			set(3, 4, Black);
			set(4, 3, Black);
            setFrontier(2, 2, true);
            setFrontier(3, 2, true);
            setFrontier(4, 2, true);
            setFrontier(5, 2, true);
            setFrontier(2, 3, true);
            setFrontier(2, 4, true);
            setFrontier(2, 5, true);
            setFrontier(3, 5, true);
            setFrontier(4, 5, true);
            setFrontier(5, 5, true);
            setFrontier(5, 3, true);
            setFrontier(5, 4, true);
		}
        
        // clone board
        Board(Board& board) : occupied(board.occupied), colors(board.colors), currentPlayer(board.currentPlayer), movesPlayed(board.movesPlayed), frontier(board.frontier) {
        }
		...
		void place(Placement placement) {
            ...
            setFrontier(placement.location.x, placement.location.y, false);
            ...
            for (auto &d : directions) {
                if (!isOccupied(placement.location.x + d.first, placement.location.y + d.second)) setFrontier(placement.location.x + d.first, placement.location.y + d.second, true);
